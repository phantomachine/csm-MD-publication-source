\begin{algorithmic}[1]
    ...
    % Policies
		...
		\pi_{1}^{j,\star},
			...
			z_{2}^{j,\star}
		\} _{j\in J},
    (b^{\star}, x^{\star}, y^{\star})
		\rangle $,
		% Settings
		$T$,
		$\omega$,
		$\phi(m,\omega) \gets \frac{m + \tau}{(1+\tau\omega)(1-\delta)}$
	}
	% Predefine object M:
	\State{Set $\mathbf{M} \gets \texttt{EmptyList}[\ ]$ }
			...
			\State{Get $u \sim \mathbf{U}[0,1]$}
				...
				% Lottery at m?
					\If{$\exists j \in J$ and
						...
						\EndIf
						% Given m (lottery outcome), eval ExPostDecisions()
						\State{$m' \gets \texttt{ExPostDecisions}(m,\omega, \mathbf{p}, \mathbf{v})$}
					\Else
